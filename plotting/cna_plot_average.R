...
pdf("mCRC_All_CNA_Diff.pdf",width=10,height=8)
...
for (pt1 in allpatients) {
  ...
  if (length(unique(sites1)) == 3)
    layout(matrix(1:(length(unique(sites1))+4),ncol=1),
	   height=c(1.5,4,rep(c(4,2),length(unique(sites1))-1),1.5))
  else 
    layout(matrix(1:(length(unique(sites1))+5),ncol=1),
	   height=c(1.5,4,rep(c(4,2),length(unique(sites1))-1),1.5,4,2))    
  par(mar=c(0,4,3,4))
  ...
  for (site1 in unique(sites1)) {
    ...
    if (site1 != "P") {
      cndiff <- cna_seg[[paste0(site1,"_diff")]] != 0
      if (any(cndiff))
	rect(cna_seg$Pos1[cndiff],-2,cna_seg$Pos2[cndiff],2,col="grey",border=NA)
    }
    ...
    if (site1 != "P") {
      ...
      if (any(cndiff)) {
	par(mar=c(4,4,0,4))
	plot(0,0,xlim=c(0,sum(as.numeric(chr_sizes))),ylim=c(0,1),
	     type="n",xaxt="n",yaxt="n",bty="n",ylab="",xlab="")
	gns <- cna2genes(cna_seg[cndiff,1:4],short_list)
	gns <- gns[!is.na(gns[,5]),1:4]
	gns <- gns[!duplicated(gns[,1]),]
	gns[,1] <- as.character(gns[,1])
	gns[,2] <- as.integer(gns[,2])
	gns <- gns[gns[,1] %in% all_diff_genes,]
	gns <- gns[order(gns$Chr,gns$Start),]
 
	if (nrow(gns) > 0) {
	  gns$Pos <- gns$Start + chr_cumsize[gns$Chr]
	  str_width <- strwidth("W",units="user",cex=1)*1.2
	  x_pos <- -str_width
	  for (i in 1:nrow(gns)){
	    x_pos1 <- gns$Pos[i]
	    if (x_pos1 - x_pos < str_width) 
	      x_pos1 <- x_pos + str_width
	    x_pos <- x_pos1
	    lines(c(gns$Pos[i],x_pos),c(1,0),lty=1)
	    axis(1,x_pos,gns[i,1],tick=FALSE,las=2,cex.axis=1,line=-1)
	  }
	}
      }
    }
